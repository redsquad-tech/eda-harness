...
class GainStageParams:
    ...
    w_in = h.Param(dtype=h.Scalar, desc="Input width in um", default=1.5)
    l_in = h.Param(dtype=h.Scalar, desc="Input length in um", default=2.0)
    nf_in = h.Param(dtype=int, desc="Input fingers", default=1)
    m_in = h.Param(dtype=int, desc="Input multiplier", default=1)
    w_load = h.Param(dtype=h.Scalar, desc="Load width in um", default=2.0)
    ...
    i_tail = h.Param(dtype=h.Scalar, desc="Tail current in A", default=1e-6)
...
def gain_stage(params: GainStageParams) -> h.Module:
    ...
    if params.load_style not in ("mirror", "diode", "cascoded"):
    ...
    npar = sky130_hdl21.Sky130MosParams(
        w=params.w_load,
        l=params.l_load,
        nf=params.nf_load,
        mult=params.m_load,
    ...
    mod.xin = diffpair(INP=mod.VINP, INN=mod.VINN, OUTP=mod.VX, OUTN=mod.VREF, TAIL=mod.tail, VDD=mod.VDD, VSS=mod.VSS)
    if params.load_style == "diode":
        ...
        # Single-ended mirror load: VREF is the diode-connected mirror reference
        # branch, VX is the high-gain inter-stage node.
        mod.m_load_ref = nmos(npar)(d=mod.VREF, g=mod.VREF, s=mod.VSS, b=mod.VSS)
        mod.m_load_out = nmos(npar)(d=mod.VX, g=mod.VREF, s=mod.VSS, b=mod.VSS)
    else:
        # Self-biased cascoded current mirror. The reference branch creates the
        # lower-device mirror bias and the upper-device cascode bias internally.
    ...
    mod.ibias_short = h.Res(r=1e-3)(p=mod.IBIAS, n=mod.tail)
    return mod
