...
VERIFICATION_PLAN = {
    "structural": {
        ...
        "extracted_metrics": ["generator call", "elaboration", "subckt name", "PMOS presence"],
    ...
    "balanced_op_tt": {
        "specification_aspect": "balanced differential-pair behavior",
        "category": "contract",
        "test_name": "run_balanced_test",
        "analysis_type": "Op",
        "extracted_metrics": ["v(outp)", "v(outn)", "abs(v(outp)-v(outn))"],
        "pass_fail_rule": "balanced input keeps output mismatch low",
        "required_corners": ["TT"],
        "required_operating_conditions": [],
        "monte_carlo_required": False,
    },
    "steer_op_tt": {
        "specification_aspect": "differential steering polarity",
        "category": "contract",
        "test_name": "run_steer_test",
        "analysis_type": "Op",
        "extracted_metrics": ["v(outp)", "v(outn)", "v(outn)-v(outp)"],
        "pass_fail_rule": "higher VINP steers less current to OUTP branch and lowers OUTP",
        "required_corners": ["TT"],
        "required_operating_conditions": [],
        "monte_carlo_required": False,
    },
...
@h.paramclass
class DiffpairPParams:
    dev_in = h.Param(dtype=str, desc="SKY130 PMOS primitive name", default="PMOS_1p8V_STD")
    ...
    body_tie = h.Param(dtype=str, desc="PMOS body tie strategy: vdd or tail", default="vdd")
...
@h.paramclass
class DiffpairPBalancedTbParams:
    ...
    r_load = h.Param(dtype=h.Scalar, desc="Per-side load resistance to VSS in ohm", default=20e3)


@h.paramclass
class DiffpairPSteerTbParams:
    vdd = h.Param(dtype=h.Scalar, desc="Supply voltage in V", default=1.8)
    v_cm = h.Param(dtype=h.Scalar, desc="Input common-mode voltage in V", default=0.9)
...
@h.generator
def diffpair_p(params: DiffpairPParams) -> h.Module:
    if params.body_tie not in ("vdd", "tail"):
    ...
    pmos = _mos_primitive(params.dev_in)
    ppar = _mos_params(params.w_in, params.l_in, params.nf_in, params.m_in)

    mod = h.Module(name="DiffpairP")
    ...
    if params.use_degeneration:
        mod.rdeg_p = h.Res(r=params.r_deg)(p=mod.TAIL, n=mod.srcp)
        mod.rdeg_n = h.Res(r=params.r_deg)(p=mod.TAIL, n=mod.srcn)
    else:
        mod.short_p = h.Res(r=1e-3)(p=mod.TAIL, n=mod.srcp)
        mod.short_n = h.Res(r=1e-3)(p=mod.TAIL, n=mod.srcn)

    body_node = mod.VDD if params.body_tie == "vdd" else mod.TAIL
    mod.mp = pmos(ppar)(d=mod.OUTP, g=mod.INP, s=mod.srcp, b=body_node)
    mod.mn = pmos(ppar)(d=mod.OUTN, g=mod.INN, s=mod.srcn, b=body_node)
    return mod


def build_balanced_test(
    dut_params: DiffpairPParams,
    tb_params: DiffpairPBalancedTbParams | None = None,
    *,
    corner=h.pdk.Corner.TYP,
) -> Sim:
    tb_params = tb_params or DiffpairPBalancedTbParams()
    install = require_sky130_install()
    dut = diffpair_p(dut_params)
    ...
    class Tb:
        ...
        i_tail = h.Idc(dc=tb_params.i_tail)(p=vdd, n=tail)
...
def build_steer_test(
    dut_params: DiffpairPParams,
    tb_params: DiffpairPSteerTbParams | None = None,
    *,
    corner=h.pdk.Corner.TYP,
) -> Sim:
    tb_params = tb_params or DiffpairPSteerTbParams()
    install = require_sky130_install()
    dut = diffpair_p(dut_params)
...
def run_balanced_test(
    dut_params: DiffpairPParams | None = None,
    tb_params: DiffpairPBalancedTbParams | None = None,
    *,
    corner=h.pdk.Corner.TYP,
    sim_options=None,
):
    dut_params = dut_params or DiffpairPParams()
    sim = build_balanced_test(dut_params, tb_params, corner=corner)
    result = run_ngspice_sim(
        sim,
        sim_options if sim_options is not None else _default_ngspice_options("diffpair_p_balanced"),
    )
...
def run_steer_test(
    ...
    *,
    ...
    result = run_ngspice_sim(
        sim,
        sim_options if sim_options is not None else _default_ngspice_options("diffpair_p_steer"),
    )
    v_outp = _op_scalar(result, "v(xtop.outp)")
    v_outn = _op_scalar(result, "v(xtop.outn)")
    return {
...
def print_test_report(
    ...
    *,
    ...
    print_metrics_table(results, title="diffpair_p")
    return results


def elaborate_dut(params: DiffpairPParams | None = None) -> h.Module:
    params = params or DiffpairPParams()
    return h.elaborate(diffpair_p(params))


def export_spice(path: str | Path, params: DiffpairPParams | None = None) -> Path:
    path = Path(path)
...
def run_structural_checks(params: DiffpairPParams | None = None):
    params = params or DiffpairPParams()
    dut = diffpair_p(params)
    mod = elaborate_dut(params)
    netlist_path = Path("./tmp/diffpair_p_structural/diffpair_p.sp")
