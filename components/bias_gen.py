...
class BiasGenSpec:
    ...
    pins: tuple[str, ...] = ("VDD", "VSS", "EN", "IBIAS1", "IBIAS2")
    measurable_behaviors: tuple[str, ...] = ("startup", "current_accuracy")
...
class BiasGenParams:
    ...
    i_ref = h.Param(dtype=h.Scalar, desc="Reference current in A", default=1e-6)
    ratio_stage1 = h.Param(dtype=h.Scalar, desc="Stage-1 current multiplier", default=1.0)
    ratio_stage2 = h.Param(dtype=h.Scalar, desc="Stage-2 current multiplier", default=2.0)
    ...
    w_ref = h.Param(dtype=h.Scalar, desc="Reference width in um", default=1.0)
    l_ref = h.Param(dtype=h.Scalar, desc="Reference length in um", default=0.15)
    nf_ref = h.Param(dtype=int, desc="Reference fingers", default=1)
    m_ref = h.Param(dtype=int, desc="Reference multiplier", default=1)
    w_out = h.Param(dtype=h.Scalar, desc="Output width in um", default=1.0)
    l_out = h.Param(dtype=h.Scalar, desc="Output length in um", default=0.15)
...
def bias_gen(params: BiasGenParams) -> h.Module:
    ...
    mod.VDD, mod.VSS, mod.EN, mod.IBIAS1, mod.IBIAS2 = h.Ports(5)
    mod.iref = h.Signal(name="iref")
    r_ref = 1.8 / float(params.i_ref)

    if params.device_type == "n":
        mod.r_ref = h.Res(r=r_ref)(p=mod.VDD, n=mod.iref)
        ...
        mod.r_ref = h.Res(r=r_ref)(p=mod.iref, n=mod.VSS)
        ...
        # Disable the PMOS mirror by clamping the shared gate/ drain reference to VDD.
        # With EN low this PMOS turns on, forcing VSG -> 0 for the reference and outputs.
        mod.m_disable = sky130_hdl21.primitives.PMOS_1p8V_STD(
            _mos_params(params.w_ref, params.l_ref, params.nf_ref, params.m_ref)
        )(d=mod.iref, g=mod.EN, s=mod.VDD, b=mod.VDD)

    mod.en_bleed = h.Res(r=1e9)(p=mod.EN, n=mod.VSS)
    return mod
...
def build_startup_test(
    ...
    *,
    ...
    class Tb:
        VSS = h.Port()
        en, ibias1, ibias2, vdd = h.Signals(4)
        vvdd = h.Vpulse(
            v1=0.0,
            v2=tb_params.vdd,
            delay=0.0,
        ...
        xdut = dut(VDD=vdd, VSS=VSS, EN=en, IBIAS1=ibias1, IBIAS2=ibias2)
