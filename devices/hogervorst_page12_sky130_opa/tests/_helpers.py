...
class BaseV4SimTest(BaseV4Test):
    ...
    def assertMetricAtLeast(self, name: str, value: float, threshold: float) -> None:
    ...
    def assertMetricAtMost(self, name: str, value: float, threshold: float) -> None:
    ...
    def assertMetricApprox(self, name: str, value: float, target: float, tol: float) -> None:
        self.assertAlmostEqual(
            value,
            target,
            delta=tol,
            msg=f"{name}={value:.6g} must be within +/- {tol:.6g} of {target:.6g}",
        )
