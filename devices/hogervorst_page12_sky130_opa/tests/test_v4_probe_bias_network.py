...
class TestV4ProbeBiasNetwork(BaseV4SimTest):
    def test_probe_bias_network(self) -> None:
        ...
        payload = {
            ...
            "mont_n_mid_V": find_signal(data, exact="v(xtop.xxdut.n_mid)"),
            "mont_p_mid_V": find_signal(data, exact="v(xtop.xxdut.p_mid)"),
            "vout_V": find_signal(data, exact="v(xtop.vout)"),
        }
        payload["derived"] = {
            "gate_spread_V": payload["vgp_V"] - payload["vgn_V"],
            "core_probe_nodes_within_rails": all(
                0.0 <= payload[key] <= vdd
                for key in [
                    "vbias1_V",
                    "vbias2_V",
        ...
        self.assertTrue(payload["derived"]["core_probe_nodes_within_rails"])
