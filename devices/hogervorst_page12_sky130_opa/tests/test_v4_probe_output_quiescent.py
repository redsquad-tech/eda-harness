...
def _run_case(*, en_v: float, inf_v: float, label: str) -> dict[str, float]:
    ...
    return {
        ...
        "id_out_n_A": float(d["i(@m.xtop.xxdut.xanalog_core.xm1.msky130_fd_pr__nfet_01v8[id])"]),
        "id_out_p_A": float(d["i(@m.xtop.xxdut.xanalog_core.xm2.msky130_fd_pr__pfet_01v8[id])"]),
        "iq_total_A": abs(float(d["i(v.xtop.vvvdd)"])),
    }


def measure_output_quiescent_cases() -> dict:
...
class TestV4ProbeOutputQuiescent(BaseV4SimTest):
    def test_probe_output_quiescent_currents(self) -> None:
        payload = measure_output_quiescent_cases()
