...
class TestV3SmokeAzResearchRunner(unittest.TestCase):
    ...
    def test_mc_variants_are_runnable(self) -> None:
        executable = build_executable_variants()
        payload = {variant_id: item.runnable for variant_id, item in executable.items() if variant_id.startswith("az_h4_")}
        self.assertTrue(payload["az_h4_v1_mc_cap200_shuntp10_freq200k"])
        self.assertTrue(payload["az_h4_v2_mc_cap200_shuntp10_rtop600"])
        json.dumps(payload)
