...
def build_cases() -> list[AzMismatchCase]:
    baseline_timing = _timing(period=5e-6, tstop=60e-6, dead_time=0.5e-6)
    return [
        AzMismatchCase(
            name="baseline_cap200_shuntp10_rtop600_freq200k",
            hypothesis="Current best AZ winner; use as MC baseline.",
            frontend_params=_frontend_params(c_az=200e-15, r_vcm_top=6e2, r_vcm_bot=5.0, c_out_p=10e-15),
            timing=baseline_timing,
        ),
        AzMismatchCase(
            name="m1_cap300",
            hypothesis="Larger AZ cap may average mismatch better and reduce offset sigma.",
            frontend_params=_frontend_params(c_az=300e-15, r_vcm_top=6e2, r_vcm_bot=5.0, c_out_p=10e-15),
            timing=baseline_timing,
        ),
        AzMismatchCase(
            name="m2_cap300_shunt_both10",
            hypothesis="Larger AZ cap plus symmetric shunts may reduce mismatch asymmetry better than positive-side-only filtering.",
            frontend_params=_frontend_params(c_az=300e-15, r_vcm_top=6e2, r_vcm_bot=5.0, c_out_p=10e-15, c_out_n=10e-15),
            timing=baseline_timing,
        ),
        AzMismatchCase(
            name="m3_big_switches",
            hypothesis="Larger symmetric switches may reduce sampling mismatch and charge-injection spread.",
            frontend_params=_frontend_params(c_az=200e-15, r_vcm_top=6e2, r_vcm_bot=5.0, c_out_p=10e-15, w_sw_n=1.3, w_sw_p=2.0, nf_sw=2),
            timing=baseline_timing,
        ),
        AzMismatchCase(
            name="m4_cap300_big_switches",
            hypothesis="Combining larger AZ cap with larger switches may be the strongest mismatch-hardening branch.",
            frontend_params=_frontend_params(c_az=300e-15, r_vcm_top=6e2, r_vcm_bot=5.0, c_out_p=10e-15, w_sw_n=1.3, w_sw_p=2.0, nf_sw=2),
            timing=baseline_timing,
        ),
        AzMismatchCase(
            name="m5_rtop700",
            hypothesis="A slightly weaker top attenuation path may improve MC spread without reopening nominal behavior.",
            frontend_params=_frontend_params(c_az=200e-15, r_vcm_top=7e2, r_vcm_bot=5.0, c_out_p=10e-15),
            timing=baseline_timing,
        ),
        AzMismatchCase(
            name="m6_rtop800",
            hypothesis="Returning toward the old top resistor may reduce dynamic mismatch sensitivity.",
            frontend_params=_frontend_params(c_az=200e-15, r_vcm_top=8e2, r_vcm_bot=5.0, c_out_p=10e-15),
            timing=baseline_timing,
        ),
    ]
...
def _top_tt(case: AzMismatchCase) -> dict:
...
def _quick_mc(case: AzMismatchCase, *, samples: int) -> dict:
    params = OpampAzTopParams(frontend_az_params=case.frontend_params)
...
def _full_mc(case: AzMismatchCase, *, samples: int) -> dict:
...
def run_batch(*, outdir: str, quick_mc_samples: int = 20, full_mc_samples: int = 50, survivor_count: int = 3) -> Path:
    ...
    json_path = outroot / "mismatch_hardening.json"
    if json_path.exists():
    ...
    for case in build_cases():
        if case.name in completed and "quick_mc" in completed[case.name] and "top_tt" in completed[case.name]:
            log(f"[az_mismatch] skip quick {case.name}")
            continue
        log(f"[az_mismatch] running quick screen {case.name}")
        try:
            entry = {
                "case": case.name,
            ...
            log(
                f"[az_mismatch] done quick {case.name} "
            ...
            log(f"[az_mismatch] failed quick {case.name}: {type(err).__name__}: {err}")

        ranked = sorted(completed.values(), key=_rank_key)
        survivors = {case["case"] for case in ranked[: max(1, min(survivor_count, len(ranked)))]}
        for item in completed.values():
            item["survivor"] = item["case"] in survivors
        payload = {
            "cases": ranked,
            "survivors": [case for case in ranked if case.get("survivor")],
            "failures": failures,
            "best_case_by_priority": ranked[0]["case"] if ranked else None,
        }
        json_path.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")
        (outroot / "mismatch_hardening.md").write_text(_render_markdown(payload), encoding="utf-8")
    ...
    for case in survivors:
        if "reduced_pvt" in case and "full_mc" in case:
            log(f"[az_mismatch] skip deep {case['case']}")
            continue
        original = next(candidate for candidate in build_cases() if candidate.name == case["case"])
        log(f"[az_mismatch] running deep screen {case['case']}")
        try:
            ...
            log(
                f"[az_mismatch] done deep {case['case']} "
            ...
            log(f"[az_mismatch] failed deep {case['case']}: {type(err).__name__}: {err}")
        payload = {
            "cases": ranked,
            "survivors": [case for case in ranked if case.get("survivor")],
            "failures": failures,
    ...
    log("[az_mismatch] complete")
    return outroot


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--outdir", default="tmp/opamp_v3_az_mismatch_hardening")
