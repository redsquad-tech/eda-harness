...
class ReferenceOutputPathParams:
    ...
    w_bias_n = h.Param(dtype=h.Scalar, desc="Replica NMOS width in um", default=0.5)
    l_bias_n = h.Param(dtype=h.Scalar, desc="Replica NMOS length in um", default=1.0)
    w_bias_p = h.Param(dtype=h.Scalar, desc="Replica PMOS width in um", default=2.4)
    ...
    r_sig_n = h.Param(dtype=h.Scalar, desc="Weak signal injection resistor to NMOS gate in ohm", default=60.0e6)
    r_sig_p = h.Param(dtype=h.Scalar, desc="Weak signal injection resistor to PMOS gate in ohm", default=1.0e12)
    r_keep_n = h.Param(dtype=h.Scalar, desc="Strong gate keep resistor for NMOS gate in ohm", default=40e3)
    r_keep_p = h.Param(dtype=h.Scalar, desc="Strong gate keep resistor for PMOS gate in ohm", default=40e3)
...
def _reference_output_path_common(params: ReferenceOutputPathParams, *, expose_gates: bool) -> h.Module:
    pmos = sky130_hdl21.primitives.PMOS_1p8V_STD
    nmos = sky130_hdl21.primitives.NMOS_1p8V_STD

    mod = h.Module(name="ReferenceOutputPathMethod2Exposed" if expose_gates else "ReferenceOutputPathMethod2")
    if expose_gates:
        mod.VDRV, mod.VOUT, mod.VGN, mod.VGP, mod.VDD, mod.VSS = h.Ports(6)
    else:
    ...
    # Current architecture intentionally uses one-sided signal drive:
    # the NMOS gate gets the signal perturbation, while the PMOS gate is held almost
    # entirely by the quiescent bias network. This avoids the older wrong-law case where
    # direct/inverted signal paths fought each other through both gates.
...
def reference_output_path_method2(params: ReferenceOutputPathParams) -> h.Module:
    return _reference_output_path_common(params, expose_gates=False)


@h.generator
def reference_output_path_method2_exposed(params: ReferenceOutputPathParams) -> h.Module:
    return _reference_output_path_common(params, expose_gates=True)
