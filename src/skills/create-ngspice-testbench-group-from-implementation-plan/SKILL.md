...
Implement the testbench groups selected from `testbench_implementation_plan.md` and bring each selected group to a reproducible working state in ngspice.
...
Determine scope from the user's request and `Implementation Order`.

* If the user explicitly specifies a group name, implement only that group.
* If the user asks for all or remaining groups, implement every unfinished group in order.
* If no group selector is present, implement every unfinished group in order.
* Treat a group as complete only after a clean saved-runner execution passes the reproducibility gate. File existence alone is not completion.
* Report concise progress after each group, but do not request confirmation between selected groups.

An acceptance failure is a valid DUT result, not an infrastructure blocker. Continue independent selected groups after acceptance failures. If one group has an infrastructure failure, preserve its diagnostic log, mark it blocked, and continue groups that do not depend on it. Stop only when a shared failure prevents every remaining selected group.
...
Create these shared files once by copying `assets/run_test.py` and writing the manifest described in `references/runner-contract.md`:
...
For each selected group, create or update only its planned files:
...
## Open-Stack Preflight

Before modifying any selected group, run one aggregated preflight for the selected scope:

* confirm the DUT workspace and workspace-local temporary/results directories are writable;
...
After creating each group, run only the saved workspace entrypoint:

```bash
python tests/run_test.py <group_name>
```

After all selected groups have been implemented, use this command when a full-suite clean rerun is required:

```bash
python tests/run_test.py --all
```

Do not present direct generator/ngspice commands as the reproducible user interface. The runner must delete the selected group's old generated fixture, log, metrics, samples, and waveform outputs before invoking the generator and ngspice.
...
* the runner observed exactly one `SUMMARY`, and its run/result/failure counts match the manifest and generated CSV;
* every planned output was recreated after stale outputs were removed;
* infrastructure failure cannot leave an old metrics/waveform CSV beside a new diagnostic log.
...
SUMMARY test=<group_name> runs=<n> results=<n> fail_count=<n>
```
...
The saved runner owns pre-run cleanup for generated fixtures and declared result artifacts. After verification, delete other temporary clutter:
...
Return a concise record for all selected groups without introducing a confirmation gate. Include:

* which groups were implemented, skipped after a clean validation, blocked, or produced DUT acceptance failures;
* which files were created/updated;
* which `tests/run_test.py` command was run;
