...
* Do not require any PDK/process model files for mock generation or the mock smoke check.
...
* If process corners are listed, treat them as downstream Cadence/Spectre coverage intent only. The mock must not include process models or pretend to model process-corner effects.
...
* process corners do not require PDK models or real corner behavior in the mock;
