...
```markdown
| Fixture Group | Order | Expected Runs | Expected RESULT Rows | Optional Parser |
|---|---:|---:|---:|---|
| `<group_name>` | `<n>` | `<n>` | `<n>` | `tests/<group_name>_parse_results.py` or `none` |
```
...
Add one shared fixture naming contract:
...
| Item | Value |
|---|---|
| Shared Top-Level Fixture Subckt | `<stable_subckt_name>` |
```

Rules:

* Each testbench group is implemented by a separate HDL21 Python file.
* `tests/run_test.py` is the single saved entrypoint for running one group or the complete generated suite.
* `tests/testbench_manifest.json` must list every planned group, its order, generator, fixture, control, log, metrics, additional artifacts, parser if any, and expected run/result counts.
* A parser may only serialize or normalize measurements and pass/fail values already produced by ngspice. It must not calculate physical metrics, convert units, interpolate waveforms, apply limits, or infer pass/fail.
* Do not plan one-off shell or heredoc post-processing.
...
* Every generated group fixture must expose the exact shared top-level `.SUBCKT` name from the fixture naming contract.
* The SPICE fixture must not include/source the selected DUT/mock netlist, real DUT netlist, PDK/foundry model files, process-corner models, or Cadence/Spectre hookup comments.
* The ngspice `.control` owns selected DUT/mock binding: it must include/source the selected ngspice DUT/mock netlist and the generated SPICE fixture.
...
* For mock/development ngspice runs, do not require PDK/foundry model files or active process-corner model includes.
* Keep explicit logical corners or `configured_process_corners` for downstream Cadence/Spectre export through `cadence_export/model_bindings.toml`.
...
* Plan any deferred process-model note only in the future `.control`; do not put process-corner TODO comments into generated SPICE fixtures.
...
Implementation order defines the deterministic order for all selected groups. An explicit group selector limits execution to that group; otherwise all unfinished planned groups may be implemented sequentially without an approval gate between groups.
...
For mock/development ngspice runs, missing local PDK files is not a blocker. Preserve the process-coverage intent from `verification_plan.md`; concrete PDK bindings belong in `cadence_export/model_bindings.toml`.

State explicitly that generated SPICE fixtures must stay DUT-implementation independent: selected ngspice DUT/mock includes belong in `.control`, not in `tests/<group_name>.sp`.
...
* the shared runner and manifest are planned;
* the shared top-level fixture subckt name is defined and used by every group;
* expected run and RESULT-row counts are defined for every group;
* any structural parser has a saved path and does not compute metrics or pass/fail;
* no per-run SPICE decks are planned;
* fixtures are planned as topology plus public DUT instance only, without selected DUT/mock includes;
* selected ngspice DUT/mock binding is planned in `.control`;
